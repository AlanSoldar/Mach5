...
def steer_toward_target(car: PlayerInfo, target: Vec3) -> float:
    angle = relativePlayerDirection(car, target)
    return limit_to_safe_range(angle * 5)

def relativePlayerDirection(car: PlayerInfo, target: Vec3) -> float:
...
def relativeDirection(carPos: Vec3, carRotation:Orientation, target: Vec3) -> float:
    relative = relative_location(carPos, carRotation, target)
    angle = math.atan2(relative.y, relative.x)
